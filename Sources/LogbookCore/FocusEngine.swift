...
public enum FocusEngine {
    public static func currentSnapshot(
        ...
        if now.timeIntervalSince(lastEvent.occurredAt) > 8 * 60 {
            ...
            if let intent {
                ...
                idleGuidance = "Set a focus target when you're ready to start so Logbook can detect drift."
            }
        ...
        guard let intent, !intent.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            return FocusSnapshot(
                ...
                headline: currentTitle ?? currentSession?.contextLabel ?? "You're active, but Logbook doesn't know what you're protecting.",
                guidance: "Set one focus target. Then Logbook can tell you whether your current behavior supports it or drifts away from it.",
                confidence: 0.66,
                minutesObserved: minutesObserved,
                contextSwitches: switchCount,
                currentApp: currentApp,
    ...
    public static func dailyNarrative(
        ...
        guard !blocks.isEmpty else {
            return "Set one focus target and leave Logbook running. The product becomes useful once it can compare intention with actual behavior."
        }

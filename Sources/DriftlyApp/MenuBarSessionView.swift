...
struct MenuBarSessionView: View {
    ...
    private var footer: some View {
        VStack(spacing: 10) {
            HStack(spacing: 10) {
                menuPrimaryButton("Open Driftly") {
                    DriftlyWindowController.showMainWindow()
                }
            ...
            HStack {
                menuTertiaryButton("Quit Driftly") {
                    DriftlyWindowController.quitApp()
                }
            }
            .frame(maxWidth: .infinity, alignment: .center)
        }
    ...
    private func menuPrimaryButton(_ title: String, destructive: Bool = false, action: @escaping () -> Void) -> some View {
        Button(title, action: action)
            ...
            .foregroundStyle(destructive ? DriftlyStyle.badgeRedText : DriftlyStyle.text)
            .padding(.horizontal, 12)
            .padding(.vertical, 7)
            .background(
                Capsule()
                    .fill(destructive ? DriftlyStyle.badgeRedFill : DriftlyStyle.badgeFill)
            )
            .overlay(
                Capsule()
                    .stroke(destructive ? DriftlyStyle.badgeRedStroke : DriftlyStyle.cardStroke, lineWidth: 1)
            )
            .buttonStyle(.plain)
    }
